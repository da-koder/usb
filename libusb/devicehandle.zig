...
pub const DeviceHandle = opaque {
    ...
    pub fn claimInterface(dev_handle: *DeviceHandle, interface_number: c_int) !void {
    ...
    pub fn releaseInterface(dev_handle: *DeviceHandle, interface_number: c_int) !void {
        var res = libusb_release_interface(dev_handle, interface_number);
        return if ( res < 0 ) toError(-res);
    }
        
    // extern fn libusb_set_interface_alt_setting(dev_handle: *DeviceHandle, interface_number: c_int, alternate_setting: c_int) c_int;
    pub fn setInterfaceAltSetting(dev_handle: *DeviceHandle, interface_number: c_int, alternate_setting: c_int) !void {
    ...
    pub fn clearHalt(dev_handle: *DeviceHandle, endpoint: *Endpoint.Descriptor) !void {
    ...
    // extern fn libusb_kernel_driver_active(dev_handle: *DeviceHandle, interface_number: c_int) c_int;
    pub fn kernelDriverActive(dev_handle: *DeviceHandle, interface_number: c_int) !void {
        var res = libusb_kernel_driver_active(dev_handle, interface_number);
        return if ( res < 0 ) toError(-res);
    }
        
    // extern fn libusb_detach_kernel_driver(dev_handle: *DeviceHandle, interface_number: c_int) c_int;
    pub fn detachKernelDriver(dev_handle: *DeviceHandle, interface_number: c_int) !void {
        var res = libusb_detach_kernel_driver(dev_handle, interface_number);
        return if ( res < 0 ) toError(-res);
    }
        
    // extern fn libusb_attach_kernel_driver(dev_handle: *DeviceHandle, interface_number: c_int) c_int;
    pub fn attachKernelDriver(dev_handle: *DeviceHandle, interface_number: c_int) !void {
    ...
    pub fn setAutoDetachKernelDriver(dev_handle: *DeviceHandle, enable: bool) !void {
        var res = libusb_set_auto_detach_kernel_driver(dev_handle, @boolToInt(enable));
        return if ( res < 0 ) toError(-res) else res;
    }
        
    // extern fn libusb_control_transfer(dev_handle: *DeviceHandle, request_type: RequestType, bRequest: u8, wValue: u16, wIndex: u16, data: [*c]u8, wLength: u16, timeout: c_uint) c_int;
    pub fn controlTransfer(dev_handle: *DeviceHandle, request_type: RequestType, bRequest: u8, wValue: u16, wIndex: u16, data: [*]u8, wLength: u16, timeout: c_uint) !usize {
        var res = libusb_control_transfer(dev_handle, request_type, bRequest, wValue, wIndex, data, wLength, timeout);
        return if ( res < 0 ) toError(-res) else @bitCast(usize,res);
    }
        
    // extern fn libusb_bulk_transfer(dev_handle: *DeviceHandle, endpoint: u8, data: [*c]u8, length: c_int, actual_length: [*c]c_int, timeout: c_uint) c_int;
    pub fn bulkTransfer(dev_handle: *DeviceHandle, endpoint: *Endpoint.Discriptor, data: []u8, timeout: c_uint) !usize {
        var actual_length: c_int = undefined;
        var res = libusb_bulk_transfer(dev_handle, @as(u8,endpoint.bEndpointAddress), data.ptr, data.len, &actual_length, timeout);
        return if ( res < 0 ) toError(-res) else @bitCast(usize, actual_length);
    }
        
    // extern fn libusb_interrupt_transfer(dev_handle: *DeviceHandle, endpoint: u8, data: [*c]u8, length: c_int, actual_length: [*c]c_int, timeout: c_uint) c_int;
    pub fn interruptTransfer(dev_handle: *DeviceHandle, endpoint: *Endpoint.Discriptor, data: []u8, timeout: c_uint) !usize {
        var actual_length: c_int = undefined;
        var res = libusb_interrupt_transfer(dev_handle, @as(u8,endpoint.bEndpointAddress), data.ptr, data.len, &actual_length, timeout);
        return if ( res < 0 ) toError(-res) else @bitCast(usize,res);
    }
    ...
    inline fn libusb_get_descriptor(arg_dev_handle: *DeviceHandle, arg_desc_type: u8, arg_desc_index: u8, arg_data: []u8) callconv(.C) c_int {
    ...
    inline fn libusb_get_string_descriptor(arg_dev_handle: *DeviceHandle, arg_desc_index: u8, arg_langid: u16, arg_data: []u8) callconv(.C) c_int {
        var wIndex = arg_langid;
        var data = arg_data.ptr;
        var wValue = @as(u16, @enumToInt(Descriptor.Type.STRING) <<  8) |  @as(u16, arg_desc_index);
...
extern fn libusb_clear_halt(dev_handle: *DeviceHandle, endpoint: u8) c_int;
extern fn libusb_reset_device(dev_handle: *DeviceHandle) c_int;
extern fn libusb_alloc_streams(dev_handle: *DeviceHandle, num_streams: u32, endpoints: [*]Endpoint.AddressType, num_endpoints: c_int) c_int;
extern fn libusb_free_streams(dev_handle: *DeviceHandle, endpoints: [*]Endpoint.AddressType, num_endpoints: c_int) c_int;
